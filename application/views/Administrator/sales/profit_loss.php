...
<div id="profitLoss">
    <div class="row" style="border-bottom: 1px solid #ccc;">
        <div class="col-md-12">
            <div class="form-inline">
                ...
                <div class="form-group" style="margin-right: 15px;" v-if="selectedSearchType.value == 'product'">
                    <label>Products &nbsp;</label>
                    <v-select v-bind:options="products" v-model="selectedProduct" label="display_text"
                        placeholder="Select Product"></v-select>
                </div>

                <div class="form-group" style="margin-right: 15px;" v-if="selectedSearchType.value == 'category'">
    ...
    <div class="row" style="display:none;" :style="{display: show_report ? '' : 'none'}"
        v-if="this.searchType == 'customer' || this.searchType == 'all' ">
    ...
    <div class="row" style="display:none;" :style="{display: show_report_product ? '' : 'none'}"
        v-if="this.searchType != 'customer' && this.searchType != 'all' ">
        <div class="col-md-12" style="margin: 10px 0;">
            <a href="" @click.prevent="print_2"><i class="fa fa-print"></i> Print</a>
        </div>
        <div class="col-md-12">
            <div class="table-responsive" id="reportTable_2">
                <table id="table1">
                    <thead>
                        <tr>
                            <th>Product Id</th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="product in reportDataByProduct">
                            <td>{{ product.Product_Code }}</td>
                            <td>{{ product.Product_Name }}</td>
                            <td style="text-align:right;">{{ product.total_sale_qty }}</td>
                            <td style="text-align:right;">{{ product.Purchase_Rate | decimal }}</td>
                            <td style="text-align:right;">{{ product.total_purcase_amount | decimal }}</td>
                            <td style="text-align:right;">{{ product.total_sale_amount | decimal }}</td>
                            <td style="text-align:right;">{{ product.total_sale_amount - product.total_purcase_amount }}
                            </td>
                        </tr>
...
new Vue({
    ...
    data() {
        return {
            ...
            searchTypes: [{
                ...
                {
                    text: 'Product Wise Profit Lose',
                    value: 'product'
                },
                {
                    text: 'Category Wise Profit Lose',
                    value: 'category'
                },
    ...
    methods: {
        ...
        async getProfitLoss() {
            ...
            if (this.searchType == 'product' || this.searchType == 'category') {
                ...
                this.reportDataByProduct = await axios.post('/get_profit_loss_by_product', this.filter)
                    .then(res => {
                        return res.data;
                    })

                this.show_report_product = true;
        ...
        onChangeSearchType() {
            if (this.selectedSearchType.value == 'customer' && this.customers.length == 0) {
                ...
                this.getProducts();
            } else if (this.selectedSearchType.value == 'category' && this.categories.length == 0) {
                this.getCategories();
            }
        },
        getCustomers() {
        ...
        async print_2() {
            ...
            let reportContent = `
					<div class="container">
						<h4 style="text-align:center">Profit/Loss Report</h4 style="text-align:center">
						<div class="row">
							<div class="col-md-12 text-right">${dateText}</div>
						</div>
						<div class="row">
							<div class="col-xs-12">
								${document.querySelector('#reportTable_2').innerHTML}
							</div>
						</div>
					</div>
				`;
