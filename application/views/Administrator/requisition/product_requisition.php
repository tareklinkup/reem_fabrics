...
<div id="productTransfer">
                        ...
                        <div class="row">
                            <div class="col-md-7">
                                <div class="form-group">
                                    <label class="control-label col-md-4">Requi. date</label>
                                    <div class="col-md-8">
                                        <input type="date" class="form-control" v-model="requisition.requisition_date">
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="control-label col-md-4">Requisition by</label>
                                    <div class="col-md-8">
                                ...
                                <div class="form-group">
                                    <label class="control-label col-md-4">Requistion to</label>
                                    <div class="col-md-8">
                                        <select class="form-control"
                            ...
                            <div class="col-md-5">
                                <div class="form-group">
                                    <textarea class="form-control" style="min-height:84px" placeholder="Note"
                                        v-model="requisition.note"></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row" style="display:none" v-bind:style="{display:cart.length > 0 ? '' : 'none'}">
        <div class="col-md-12">
            <button class="btn btn-success pull-right" v-on:click="saveProductRequisition">Save</button>
        </div>
    </div>
</div>
...
new Vue({
    ...
    data() {
        return {
            requisition: {
                requisition_id: parseInt('<?php echo $requisitionId;?>'),
    ...
    methods: {
        ...
        saveProductRequisition() {
            if (this.requisition.requisition_date == null) {
                alert('Select requisition date');
                return;
            }

            if (this.selectedEmployee == null) {
                alert('Select requisition by');
                return;
            }
            ...
            this.requisition.total_amount = this.cart.reduce((p, c) => {
                return p + +c.total
            }, 0);

            this.requisition.requisition_by = this.selectedEmployee.Employee_SlNo;
            this.requisition.requisition_to = this.selectedBranch.brunch_id;
            ...
            let url = '/add_product_requisition';
            if (this.requisition.requisition_id != 0) {
                url = '/update_product_requisition';
            }
        ...
        async getRequisition() {
            ...
            let requisition = await axios.post('/get_requisitions', {
                requisitionId: this.requisition.requisition_id
            }).then(res => {
                return res.data[0];
            })
            ...
            this.selectedEmployee = {
                Employee_SlNo: requisition.requisition_by,
                Employee_Name: requisition.requisition_by_name
            }

            this.selectedBranch = {
                brunch_id: requisition.requisition_to,
                Brunch_name: requisition.requisition_to_name
            }

            let requisitionDetails = await axios.post('/get_requisition_details', {
                requisitionId: this.requisition.requisition_id
            }).then(res => {
                return res.data;
            })

            this.cart = requisitionDetails.map(td => {
                let product = {
